...
<body>
	...
	<section>
		<h2>학사 정보 변경 화면</h2>
		<form name="iu_form" method="post" action="stud0210_update_process.jsp">
			<table id="iu_table">
				<tr>
					<th>학번</th>
				...
				<tr>
					<th>취미</th>
					<td>
						<input type="checkbox" name="hobby" value="프로그램" <%for(String h:hobbys){if(h.equals("프로그램")){%> checked <%}} %>>프로그램
						<input type="checkbox" name="hobby" value="독서" <%for(String h:hobbys){if(h.equals("독서")){%> checked <%}} %>>독서
						<input type="checkbox" name="hobby" value="등산" <%for(String h:hobbys){if(h.equals("등산")){%> checked <%}} %>>등산
						<input type="checkbox" name="hobby" value="여행" <%for(String h:hobbys){if(h.equals("여행")){%> checked <%}} %>>여행
						<input type="checkbox" name="hobby" value="낚시" <%for(String h:hobbys){if(h.equals("낚시")){%> checked <%}} %>>낚시
						<input type="checkbox" name="hobby" value="영화보기" <%for(String h:hobbys){if(h.equals("영화보기")){%> checked <%}} %>>영화보기
						<input type="checkbox" name="hobby" value="잠자기" <%for(String h:hobbys){if(h.equals("잠자기")){%> checked <%}} %>>잠자기
						<input type="checkbox" name="hobby" value="멍때리기" <%for(String h:hobbys){if(h.equals("멍때리기")){%> checked <%}} %>> 멍때리기
					</td>
				</tr>
				<tr>
					<th>주소</th>
